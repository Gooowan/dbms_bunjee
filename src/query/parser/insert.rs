...
impl InsertParser {
    ...
    pub fn parse_and_execute(
        ...
        // Parse values
        let values = self.parse_values(&tokens[values_start + 1..])?;
        ...
        let block = if let Some(block) = storage_blocks.last_mut() {
            if block.count() < 1000 { // Assuming max 1000 records per block
                block
            } else {
        ...
        if block.insert(record) {
            ...
            Err(QueryError::InternalError("Failed to insert record".to_string()))
        }
    }
    ...
    fn parse_values(&self, tokens: &[&str]) -> Result<Vec<String>, QueryError> {
        ...
        for token in tokens {
            if token.starts_with('(') {
                ...
                value.push_str(&token[..token.len() - 1]);
                if !value.is_empty() {
